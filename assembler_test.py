...
def test_loadi_encodes_correctly():
    ...
    assert prog[0] == 0x1 << 12 | 0x5 << 9 | 0b1111011 << 1
...
def test_lui_encodes_correctly():
    src = ["LUI R7 #123"]
    prog = assemble(src)
    assert prog[0] == 0x2 << 12 | 0x7 << 9 | 0b1111011 << 1
    ...
    src = ["LUI R5 #257"]
    prog = assemble(src)
    assert prog[0] == 0x2 << 12 | 0x5 << 9 | 0b1 << 1

    # What about a negative immediate?
    src = ["LUI R5 #-1"]
    prog = assemble(src)
    assert prog[0] == 0x2 << 12 | 0x5 << 9 | 0b11111111 << 1
...
def test_load_encodes_correctly():
    ...
    src = ["LOAD R2 [R3 + #65]"]
    prog = assemble(src)
    assert prog[0] == 0x3 << 12 | 0x2 << 9 | 0x3 << 6 | 0x1

    # What about a negative offset?
    src = ["LOAD R2 [R3 + #-5]"]
    prog = assemble(src)
    assert prog[0] == 0x3 << 12 | 0x2 << 9 | 0x3 << 6 | 0b111011

    # What about no offset?
    src = ["LOAD R2 [R3]"]
    prog = assemble(src)
    assert prog[0] == 0x3 << 12 | 0x2 << 9 | 0x3 << 6 | 0x0


def test_store_encodes_correctly():
    src = ["STORE R1, [R2 + #5]"]
    prog = assemble(src)
    assert prog[0] == 0x4 << 12 | 0x1 << 9 | 0x2 << 6 | 0x5


def test_addi_encodes_correctly():
    src = ["ADDI R1, R2, #5"]
    prog = assemble(src)
    assert prog[0] == 0x5 << 12 | 0x1 << 9 | 0x2 << 6 | 5
...
def test_or_encodes_correctly():
    src = ["OR R1, R2, R3"]
    prog = assemble(src)
    assert prog[0] == 0x9 << 12 | 0x1 << 9 | 0x2 << 6 | 0x3 << 3


def test_xor_encodes_correctly():
    src = ["XOR R1, R2, R3"]
    prog = assemble(src)
    assert prog[0] == 0xA << 12 | 0x1 << 9 | 0x2 << 6 | 0x3 << 3


def test_shft_encodes_correctly():
    src = ["SHFT R3, R4, R5"]
    prog = assemble(src)
    assert prog[0] == 0xB << 12 | 0x3 << 9 | 0x4 << 6 | 0x5 << 3


def test_beq_encodes_with_forward_label():
    src = ["BEQ TARGET", "LOADI R0, #0", "TARGET:", "HALT"]
    prog = assemble(src)
    # Bcond encoding: opcode(4) | imm(8) | cc(2) | zero(2)
    # opcode = 0xC, cc = 0b00 (BEQ), offset = 1
    expected = (0xC << 12) | (1 << 4) | (0b00 << 2)
...
def test_beq_encodes_with_backward_label():
    ...
    prog = assemble(src)
    # labels['LOOP'] = 0, pc = 1 for BEQ
    # offset = (0 - 1 - 1) & 0xFF
    expected_offset = (0 - 1 - 1) & 0xFF
    expected = (0xC << 12) | (expected_offset << 4) | (0b00 << 2)
...
def test_bne_encodes_with_forward_label():
    ...
    prog = assemble(src)
    # Bcond encoding: opcode(4) | imm(8) | cc(2) | zero(2)
    # opcode = 0xC, cc = 0b01 (BNE), offset = 1
    expected = (0xC << 12) | (1 << 4) | (0b01 << 2)
    assert prog[0] == expected  # check first instruction
...
def test_bne_encodes_with_backward_label():
    ...
    prog = assemble(src)
    # labels['LOOP'] = 0, pc = 1 for BNE
    # offset = (0 - 1 - 1) & 0xFF
    expected_offset = (0 - 1 - 1) & 0xFF
    expected = (0xC << 12) | (expected_offset << 4) | (0b01 << 2)
    assert prog[1] == expected  # check second instruction


def test_blt_encodes_with_forward_label():
    src = ["BLT TARGET", "LOADI R0, #0", "TARGET:", "HALT"]
    prog = assemble(src)
    # Bcond encoding: opcode(4) | imm(8) | cc(2) | zero(2)
    # opcode = 0xC, cc = 0b10 (BLT), offset = 1
    expected = (0xC << 12) | (1 << 4) | (0b10 << 2)
    assert prog[0] == expected


def test_blt_encodes_with_backward_label():
    src = ["LOOP:", "LOADI R0, #0", "BLT LOOP"]
    prog = assemble(src)
    expected_offset = (0 - 1 - 1) & 0xFF
    expected = (0xC << 12) | (expected_offset << 4) | (0b10 << 2)
    assert prog[1] == expected


def test_bge_encodes_with_forward_label():
    src = ["BGE TARGET", "LOADI R0, #0", "TARGET:", "HALT"]
    prog = assemble(src)
    # Bcond encoding: opcode(4) | imm(8) | cc(2) | zero(2)
    # opcode = 0xC, cc = 0b11 (BGE), offset = 1
    expected = (0xC << 12) | (1 << 4) | (0b11 << 2)
    assert prog[0] == expected


def test_bge_encodes_with_backward_label():
    src = ["LOOP:", "LOADI R0, #0", "BGE LOOP"]
    prog = assemble(src)
    expected_offset = (0 - 1 - 1) & 0xFF
    expected = (0xC << 12) | (expected_offset << 4) | (0b11 << 2)
    assert prog[1] == expected
...
def test_b_encodes_with_forward_label():
    ...
    # offset = (1 - 0 - 1) & 0xFF = 0
    expected_offset = 0
    expected = (0xD << 12) | expected_offset
    assert prog[0] == expected
...
def test_call_encodes_correctly():
    ...
    expected = (0xE << 12) | (expected_offset << 4)
    prog = assemble(src)
    assert prog[1] == expected
...
def test_halt_encodes_correctly():
    src = ["HALT"]
    prog = assemble(src)
    assert prog[0] == 0x0 << 12


def test_mov_encodes_as_addi_zero():
    """MOV Rd, Ra is a pseudo-instruction that encodes as ADDI Rd, Ra, #0."""
    from instruction_set import ISA

    addi_opcode = ISA["ADDI"]["opcode"]
    prog = assemble(["MOV R3, R1"])
    expected = (addi_opcode << 12) | (3 << 9) | (1 << 6)
    assert prog[0] == expected


def test_mov_in_context():
    """MOV works correctly in a program alongside real instructions."""
    from cpu import make_cpu

    prog = assemble(["LOADI R1, #42", "MOV R2, R1", "HALT"])
    c = make_cpu(prog)
    c.tick()  # LOADI R1
    c.tick()  # MOV R2, R1
    assert c.get_reg(2) == 42
    assert c.get_reg(1) == 42  # source unchanged
